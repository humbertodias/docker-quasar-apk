...
RUN apt update && apt install sudo git curl wget zip unzip build-essential file apt-utils jq -y
...
RUN sudo npm install -g npm@$NPM_VERSION && \
    ...
    sudo npm install -g @quasar/cli@$QUASAR_CLI_VERSION
...
ENV ANDROID_SDK_VERSION=8092744
